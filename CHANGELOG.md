...
- `parseBatcherEvent` rejected account (`G…`) callers, rejected the bare-scalar `init` / `bat_abort` payloads the contract actually emits, and accepted any string as a `u32`; it now shares the strict decoder's validation and still returns `null` on failure (#793)

### Added
- Fail-closed release profile verification in CI (#780): `scripts/check-release-profile.sh` asserts every `[profile.release]` invariant from `docs/release-profile-verification.md`, rejects per-package release overrides, and checks release WASMs carry no `.debug_*` sections. `ci.yml` and `deploy.yml` run it before and after a `RUSTFLAGS="-D warnings"` release build, and CI uploads WASM artifacts only when every gate passes. Self-tests in `scripts/test-check-release-profile.sh`; `make check-release-profile` runs it locally
- Strict batcher event bindings (#793): `decodeBatcherEvent` / `decodeBatcherEvents` throw typed `BatcherEventDecodeError`s (`NotBatcherEvent`, `MalformedTopics`, `UnknownAction`, `MalformedPayload`, `InvalidAddress`, `InvalidU32`, `MissingTxHash`, `InvalidTxHash`) for malformed event logs and attach `txHash`, `correlationId`, `eventId`, `ledger`, and `contractId`; `correlateBatcherEvents` groups a stream by batch invocation
- `docs/sdk-bump-playbook.md` — Soroban SDK bump playbook establishing mandatory upgrade procedures, version compatibility matrix, release WASM verification, and rollback strategy (#852)
- `ops/README.md` — Comprehensive operations runbook index linking all deployment checklists, key rotations, upgrade patterns, rollbacks, and incident response procedures (#851)
- `docs/pause-freeze-decision.md` — Architectural Decision Record (ADR) on per-account self-custodial circuit breaker (`pause`/`unpause`) and deliberate rejection of global freeze backdoors (#845)
- Documented §4.14 Emergency Pause and Freeze threats (T-50…T-52) and security controls in `docs/threat-model.md` (#845)
